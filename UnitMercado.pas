...
begin
      // Cria as configurações de formato
    ...
    FormatSettings.ThousandSeparator := '.';
     ...
     frame.lblValor.text := FormatFloat('R$ #,##0.00', valor, FormatSettings);
...
procedure TFrmMercado.btnAdicionarClick(Sender: TObject);
begin
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                begin
                  try
                     // Obter o preço como string
                        precoStr := fieldbyname('preco').AsString;
                        // Trocar ponto por vírgula, se necessário
                        precoStr := StringReplace(precoStr, '.', ',', [rfReplaceAll]);
                        ...
                        begin
                            ShowMessage('Erro ao converter valores: ' + E.Message + ' - Valor: ' + precoStr);
                        end;
                    end;
                end);
                Next;
...
begin
    ...
    begin
        ...
        begin
            ...
            var
                vlEntregaStr: string;
                vlCompraMinStr: string;
                vlEntrega: Double;
                vlCompraMin: Double;
            begin
                    // Obtem os valores como string
                    vlEntregaStr := fieldbyname('vl_entrega').AsString;
                    vlCompraMinStr := fieldbyname('vl_compra_min').AsString;
                    // Verifique o formato e ajuste se necessário
                    if Pos('.', vlEntregaStr) > 0 then
                    vlEntregaStr := StringReplace(vlEntregaStr, '.', ',', [rfReplaceAll]);
                    if Pos('.', vlCompraMinStr) > 0 then
                    vlCompraMinStr := StringReplace(vlCompraMinStr, '.', ',', [rfReplaceAll]);
                    // Tenta converter as strings para Double
                try
                    vlEntrega := StrToFloat(vlEntregaStr, FormatSettings);
                except
                    on E: EConvertError do
                    begin
                        ShowMessage('Erro ao converter a taxa de entrega: ' + E.Message + ' - Valor: ' + vlEntregaStr);
                        Exit;
                    end;
                end;
                try
                    vlCompraMin := StrToFloat(vlCompraMinStr, FormatSettings);
                except
                    on E: EConvertError do
                    begin
                        ShowMessage('Erro ao converter a compra mínima: ' + E.Message + ' - Valor: ' + vlCompraMinStr);
                        Exit;
                    end;
                end;
                // Atualiza os labels com os valores formatados
                lblTitulo.Text := fieldbyname('nome').AsString;
                lblEndereco.Text := fieldbyname('endereco').AsString;
                lblEntrega.Text := 'Tx. Entrega: ' + FormatFloat('R$#,##0.00', vlEntrega, FormatSettings);
                lblEntrega.TagFloat := vlEntrega;
                lblPedMin.Text := 'Compra Mín: ' + FormatFloat('R$#,##0.00', vlCompraMin, FormatSettings);
            end);
        end;
