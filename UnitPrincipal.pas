...
begin
  ...
  FormatSettings.ThousandSeparator := '.';
        ...
        txt.Text := 'Taxa de entrega: ' + FormatFloat('R$ #,##0.00', tx_entrega, FormatSettings);

        txt := TListItemText(Objects.FindDrawable('txtCompraMin'));
        txt.Text := 'Compra mínima: ' + FormatFloat('R$ #,##0.00', vl_min_ped, FormatSettings);
    end;
end;
...
//Atualizar listagem de mercados:
        ...
        begin
            ...
            begin
              TThread.Synchronize(TThread.CurrentThread, procedure
                begin
                    try
                        // Pega o valor como string e troca o ponto pela vírgula
                        taxa := StrToFloat(StringReplace(fieldbyname('vl_entrega').AsString, '.', ',', [rfReplaceAll]), FormatSettings);
                        compraMin := StrToFloat(StringReplace(fieldbyname('vl_compra_min').AsString, '.', ',', [rfReplaceAll]), FormatSettings);
