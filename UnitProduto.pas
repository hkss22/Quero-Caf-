...
begin
    ...
    begin
        TDialogService.MessageDialog('Você só pode adicionar itens de um estabelecimento por vez. Deseja esvaziar a sacola e adicionar esse item?',
       ...
       // sleep(1000);
        ...
        begin
            ...
            begin
                ...
                //ShowMessage('Valor original do preço: ' + precoStr);

                // Verifique o formato e ajuste se necessário
                  if Pos('.', precoStr) > 0 then
                  precoStr := StringReplace(precoStr, '.', ',', [rfReplaceAll]);

                // Tenta converter as strings para Double
                try
                    preco := StrToFloat(precoStr, FormatSettings);
                except
                    on E: EConvertError do
                    begin
                        ShowMessage('Erro ao converter a taxa de entrega: ' + E.Message + ' - Valor: ' + precoStr);
                        Exit;
                    end;
                end;
                ...
                lblValor.Text := FormatFloat('R$#,##0.00', preco, FormatSettings);
                lblValor.TagFloat := preco;
